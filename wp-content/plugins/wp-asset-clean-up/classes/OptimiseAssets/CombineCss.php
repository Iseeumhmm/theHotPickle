		...
		if ($skipIfFileExists || ! file_exists($localFinalCssFile)) {
			...
			foreach ($localAssetsPaths as $assetHref => $localAssetsPath) {
				if ($cssContent = trim(FileSystem::file_get_contents($localAssetsPath))) {
					...
					$finalAssetsContents .= '/*! '.str_replace(ABSPATH, '/', $localAssetsPath)." */\n";
					$finalAssetsContents .= OptimizeCss::maybeFixCssContent($cssContent, $pathToAssetDir . '/') . "\n";
				}
			}

			$finalAssetsContents = trim(self::moveImportsToTop($finalAssetsContents));
